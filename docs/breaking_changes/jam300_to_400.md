...
user = jam.authenticate(token, via="jwt")   # -> User(id="1", email="user@example.com")
```
...
New `jam.authorize(subject, permission)` and the `[jam.authz]` config section.
Deny by default; rules are declarative:
...
Or pass a custom module:
```toml
[jam.authz]
module = "my_project.authz.MyPolicy"
```
