...
For facade authentication, use a JWT configured for JWS+JWE and pass the
nested token to `jam.authenticate(encrypted, via="jwe")`. The facade rejects
JWE-only data, verifies the inner signature, and validates `exp` and `nbf`.
...
* `JamJWTInvalidClaim` - `exp` or `nbf` is not a finite NumericDate.
...
When claim validation is enabled, `exp` and `nbf` must be finite integers or
floats; booleans, strings, NaN, and infinities are rejected.
`exp == current_time` is expired. The same checks apply when registered claims
are supplied through the custom `payload` dictionary and to the signed payload
inside an encrypted JWT.
