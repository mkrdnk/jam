...
More modern RSA signature scheme with probabilistic salt. Jam 4.2.2 creates
standards-compatible signatures using a salt equal to the hash digest size.
Verification also accepts maximum-length PSS salts emitted by earlier Jam
releases so existing tokens remain usable during upgrades.
...
| `RSA-OAEP` | RSA | OAEP with SHA-1, as defined by RFC 7518 |
| `RSA-OAEP-256` | RSA | OAEP with SHA-256 (recommended) |

Encrypt CEK with RSA public key, decrypt with private key.

Jam releases before 4.2.2 used SHA-256 under the `RSA-OAEP` name. New tokens
use the RFC-defined SHA-1 parameters; decryption retains a SHA-256 fallback
for tokens produced by those earlier releases. Use `RSA-OAEP-256` when
SHA-256 OAEP is required for new deployments.
...
```
RS256/PS256 + RSA-OAEP-256 + A128CBC-HS256  → Basic RSA encryption
RS512/PS512 + RSA-OAEP-256 + A256CBC-HS512  → High security RSA encryption
```
...
| `RSA-OAEP` | ✓ | RFC 7518 compatibility, uses SHA-1 |
| `RSA-OAEP-256` | ✓ | Recommended for new RSA encryption |
