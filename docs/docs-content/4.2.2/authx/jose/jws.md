...
For `PS256`, `PS384`, and `PS512`, Jam 4.2.2 creates standard RSA-PSS
signatures with a salt equal to the hash digest size. Verification additionally
accepts maximum-length salts produced by older Jam versions, so previously
issued tokens continue to verify.
