...
* `header`: `dict[str, Any] | None = None` - Additional header fields. It
  cannot override the configured `alg` or `enc`.
...
* `JamJWEEncryptionError` - Encryption failed, including an attempt to
  override reserved `alg` or `enc` fields.
...
`jam.authenticate(token, via="jwe")` accepts only a nested signed-and-encrypted
JWT. After decryption Jam verifies the inner JWS signature and validates its
registered claims, including `exp` and `nbf`. A JWE-only payload can still be
decrypted through the lower-level `jam.jwt.decrypt()` or `jam.jwe.decrypt()`,
but it is rejected as an authentication credential because encryption alone
does not establish an issuer. Missing nested JWS configuration raises
`JamConfigurationError` with
`configuration.jwe.authentication_requires_jws`; an authenticated payload
...
* `header`: `dict[str, Any] | None = None` - Additional JWE header fields.
  `alg` and `enc` are reserved and cannot override the instance configuration.
...
The protected header must contain the exact `alg` and `enc` configured on the
recipient. Missing or different values are rejected before key or content
decryption, preventing algorithm downgrade. Select accepted algorithms by
constructing the appropriate `JWE` instance; token headers do not select them.
