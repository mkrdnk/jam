...
pip install "jamlib[json]"      # JSON sessions and lists
...
Session backends are loaded lazily. Importing `jam.sessions` or
`BaseSessionModule` does not require TinyDB or Redis; install the `json` or
`redis` extra only when selecting that backend.
