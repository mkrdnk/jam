...
* `serializer`: `BaseEncoder | type[BaseEncoder] = JsonEncoder` - JSON
  serializer used by token modules and as the fallback for sessions.
...
| `jam.lists` | `dict[str, BaseList]` | `[jam.lists]` |
| `jam.macaroon` | `Macaroon` | `[jam.macaroon]` |
| `jam.saml` | `SAML` | `[jam.saml]` |
...
* `via`: `JamIssueType` - Required credential type: `"jwt"`, `"paseto"`,
...
* `via`: `JamAuthType` - Required credential type: `"jwt"`, `"jwe"`,
  `"paseto"`, `"session"`, `"macaroon"`, or `"saml"`.
...
`via="jwe"` is an authentication mode for nested signed-and-encrypted JWTs.
It rejects encrypted-only data, verifies the inner JWS, and validates its
...
Use `Jam` as a context manager when configured modules create file, Redis, or
HTTP resources:

```python
with Jam(config="config.toml") as jam:
    token = jam.issue({"id": "1"}, via="jwt")
```

`Jam.close()` closes owned synchronous resources once per module reference.
It does not close caller-owned Redis clients.
...
`await jam.aclose()` is the explicit equivalent. It also closes configured
named token lists, while leaving caller-owned clients open.
