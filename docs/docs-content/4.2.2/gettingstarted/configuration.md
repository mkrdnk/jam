...
Python standard library `json` module. The root serializer is also the
fallback for session storage. A serializer configured explicitly in
`[jam.session]` takes precedence for that backend.
