...
Token-list validation and configuration may use these contextual codes:

| Code | Exception | Description |
| --- | --- | --- |
| `validation.lists.invalid_token` | `JamValidationError` | A token is empty, not a string, or cannot be encoded as valid UTF-8. |
| `configuration.lists.invalid_legacy_raw_keys` | `JamConfigurationError` | `legacy_raw_keys` is not a boolean. |
...
| `JamJWTInvalidClaim` | `jwt.invalid_claim` | An `exp` or `nbf` claim is not a finite NumericDate. |
...
| `JamSessionExpired` | `sessions.expired` | The persisted session has expired. |
| `JamSessionNotYetValid` | `sessions.not_yet_valid` | The persisted session is not valid yet. |
| `JamSessionInvalidClaim` | `sessions.invalid_claim` | A session `exp` or `nbf` claim is not a finite NumericDate. |
...
| `configuration.jwe.authentication_requires_jws` | `JamConfigurationError` | Facade JWE authentication is configured without a nested JWS. |
