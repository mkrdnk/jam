...
* `jam.authenticate(token, via="jwt" | "jwe" | "paseto" | "session")` — validate a token.
  `via` is required; credential types are not detected automatically.
...
`await jam.issue(..., via=...)` and `await jam.authenticate(..., via=...)` for high-level
...
[Authorization](/4.0.0/authz/rules) for the complete configuration syntax.
...
query parameters. Every adapter requires
`via="jwt" | "jwe" | "paseto" | "session"`; credential types are not detected
automatically.
...
backend = JamAuthBackend(jam, sources=sources, via="jwt")
```
...
plugin = JamPlugin(jam, sources=sources, via="jwt")
